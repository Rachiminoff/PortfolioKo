...
function Vault() {
  ...
  const toggleFavorite = async (item: VaultItem) => {
    ...
    setVaultItems((prev) =>
      prev.map((i) => (i.id === item.id ? { ...i, favorite: newFavorite } : i)),
    );
  ...
  const filteredAndSortedItems = useMemo(() => {
    ...
    if (filterChip === 'pdf') {
      items = items.filter((item) => !item.type.toLowerCase().includes('epub'));
  ...
  const categories = Array.from(new Set(vaultItems.map((item) => item.category ?? 'General')));
  ...
  const getFolderCovers = (category: string) => {
    const items = vaultItems.filter((item) => (item.category ?? 'General') === category);
    return items
        ...
        >
          ...
          <div className="vault-book-content">
            ...
            <div className="vault-meta-row">
              ...
              {item.created_at && <span>{new Date(item.created_at).getFullYear()}</span>}
              {item.series && <span className="vault-series-badge">{item.series}</span>}
            </div>

            {item.description && (
              <div className="vault-description-preview">{item.description}</div>
            )}
      ...
      >
        ...
        <div className="vault-book-content">
          ...
          <div className="vault-description-dropdown">
            <button className="vault-description-toggle" onClick={() => toggleDescription(item.id)}>
              Description
  ...
  const renderSeriesGroup = (seriesName: string, items: VaultItem[]) => {
    ...
    return (
      <div key={seriesName} className="vault-series-group">
        <button className="vault-series-header" onClick={() => toggleSeries(seriesName)}>
          <div className="vault-series-header-left">
            {coverImage && <img src={coverImage} alt={seriesName} className="vault-series-cover" />}
            <span className="vault-series-icon">
              <Icon icon="mdi:bookshelf" />
            </span>
            <h3>{seriesName}</h3>
            <span className="vault-series-count">({items.length})</span>
          </div>
  ...
  return (
    <div
      className={`vault-container ${activeCard ? 'vault-active' : ''} ${isPageVisible ? 'vault-visible' : ''}`}
    >
      {/* HEADER */}
      <div className="vault-header">
        ...
        <div className="vault-controls-bar">
          <div className="vault-search-wrapper">
            ...
            />
            {searchQuery && (
              <button className="vault-search-clear" onClick={() => setSearchQuery('')}>
                <Icon icon="mdi:close" />
              </button>
            )}
          </div>
      ...
      <div className="vault-main">
        {loadError ? (
          <div className="vault-error-state">
            ...
            <button className="vault-empty-clear-btn" onClick={() => fetchVaultItems()}>
              Try again
            </button>
          </div>
        ) : loading ? (
                ...
                <div className="vault-continue-card">
                  ...
                  <div className="vault-continue-info">
                    ...
                    <button
                      ...
                      onClick={() =>
                        openViewer(continueReading.link, continueReading.type, continueReading)
                      }
                    >
                      Open
                    </button>
                      ...
                      <div className="vault-folder-cover-area">
                        {covers.length > 0 ? (
                          <div className="vault-folder-cover-collage">
                            ...
                            {covers.length < 6 &&
                              Array.from({ length: 6 - covers.length }).map((_, idx) => (
                ...
                <div className="vault-category-header">
                  <div className="vault-category-header-left">
                    ...
                    <span className="vault-item-count">
                      ...
                      {filteredAndSortedItems.length === 1 ? 'Entry' : 'Entries'}
                    </span>
                  </div>
                  <button
      ...
      {/* VIEWER OVERLAY */}
      {viewerUrl && viewerType === 'pdf' && <PDFViewer url={viewerUrl} onClose={closeViewer} />}

      {viewerUrl && viewerType === 'epub' && <EPUBViewer url={viewerUrl} onClose={closeViewer} />}
    </div>
  );
}
