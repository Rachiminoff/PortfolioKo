...
function Insights({ onClose }: InsightsProps) {
  ...
  const filteredAndSortedPosts = useMemo(() => {
    ...
    if (selectedCategory !== 'all') {
      items = items.filter(
        (item) =>
          item.category === selectedCategory || item.tags.some((tag) => tag === selectedCategory),
      );
    }
    ...
    return markdown
      ...
      .map((block) => {
        ...
        if (/^#{1,6}\s/.test(trimmed)) {
          ...
          return match
            ? `<h${match[1].length}>${escape(match[2])}</h${match[1].length}>`
            : `<p>${escape(trimmed)}</p>`;
        }
        if (/^>\s/.test(trimmed))
          return `<blockquote>${escape(trimmed.replace(/^>\s?/gm, ''))}</blockquote>`;
        if (/^(?:-\s|\*\s)/m.test(trimmed)) {
          return `<ul>${trimmed
            .split('\n')
            .map((line) => `<li>${escape(line.replace(/^(?:-\s|\*\s)/, ''))}</li>`)
            .join('')}</ul>`;
        }
  ...
  const switchEditorMode = (mode: 'rich' | 'markdown') => {
    ...
    const nextContent =
      editorMode === 'rich' ? htmlToMarkdown(editorRef.current?.innerHTML || '') : editorContent;
      ...
      <div className="insights-container insights-locked">
        <div className="insights-lock-screen">
          ...
          <form onSubmit={handleUnlock} className="insights-lock-form">
            ...
            <button type="submit" className="insights-unlock-btn" disabled={isLoading || !password}>
              {isLoading ? (
              ...
              <div className="insights-reader-hero-badges">
                ...
                {selectedPost.tags &&
                  selectedPost.tags.slice(0, 3).map((tag) => (
                    <span key={tag} className="insights-reader-tag-badge">
                      {tag}
                    </span>
                  ))}
              </div>
          ...
          <article className="insights-reader-content-wrapper">
            <div className="insights-reader-content-inner">
              ...
              {selectedPost.excerpt && (
                <div className="insights-reader-excerpt">{selectedPost.excerpt}</div>
              )}
                ...
                <ReactMarkdown
                  ...
                  components={{
                    h1: ({ children, ...props }: any) => (
                      <h1 className="insights-reader-heading-1" {...props}>
                        {children}
                      </h1>
                    ),
                    h2: ({ children, ...props }: any) => (
                      <h2 className="insights-reader-heading-2" {...props}>
                        {children}
                      </h2>
                    ),
                    h3: ({ children, ...props }: any) => (
                      <h3 className="insights-reader-heading-3" {...props}>
                        {children}
                      </h3>
                    ),
                    h4: ({ children, ...props }: any) => (
                      <h4 className="insights-reader-heading-4" {...props}>
                        {children}
                      </h4>
                    ),
                    h5: ({ children, ...props }: any) => (
                      <h5 className="insights-reader-heading-5" {...props}>
                        {children}
                      </h5>
                    ),
                    h6: ({ children, ...props }: any) => (
                      <h6 className="insights-reader-heading-6" {...props}>
                        {children}
                      </h6>
                    ),
                    p: ({ children, ...props }: any) => (
                      <p className="insights-reader-paragraph" {...props}>
                        {children}
                      </p>
                    ),
                    ul: ({ children, ...props }: any) => (
                      <ul className="insights-reader-list insights-reader-list-ul" {...props}>
                        {children}
                      </ul>
                    ),
                    ol: ({ children, ...props }: any) => (
                      <ol className="insights-reader-list insights-reader-list-ol" {...props}>
                        {children}
                      </ol>
                    ),
                    li: ({ children, ...props }: any) => (
                      <li className="insights-reader-list-item" {...props}>
                        {children}
                      </li>
                    ),
                    blockquote: ({ children, ...props }: any) => (
                      <blockquote className="insights-reader-blockquote" {...props}>
                        {children}
                      </blockquote>
                    ),
                      ...
                      return !inline ? (
                        <div className="insights-reader-code-block-wrapper">
                          <div className="insights-reader-code-header">
                            <span className="insights-reader-code-language">
                              {match ? match[1] : 'code'}
                            </span>
                            <button
                    ...
                    table: ({ children, ...props }: any) => (
                      <div className="insights-reader-table-wrapper">
                        <table className="insights-reader-table" {...props}>
                          {children}
                        </table>
                      </div>
                    ),
                    a: ({ href, children, ...props }: any) => (
                      <a
                        href={href}
                        className="insights-reader-link"
                        target="_blank"
                    ...
                    hr: (props: any) => <hr className="insights-reader-hr" {...props} />,
                  }}
                >
                ...
                <div className="insights-reader-tags-footer">
                  ...
                  <div className="insights-reader-tags-list">
                    {selectedPost.tags.map((tag) => (
                      <span key={tag} className="insights-reader-tag-pill">
                        {tag}
                      </span>
                    ))}
                  </div>
  ...
  return (
    <div className="insights-container">
        ...
        <div className="insights-hero-content">
          <div className="insights-hero-header">
            ...
            <p className="insights-hero-subtitle">
              A little archive of things I've built, noticed, read, and spent far too much time
              thinking about.
            </p>
          </div>
      ...
      <div className="insights-controls-section">
        <div className="insights-controls-bar">
          <div className="insights-search-wrapper">
            ...
            />
            {searchQuery && (
              <button className="insights-search-clear" onClick={() => setSearchQuery('')}>
                ✕
              </button>
            )}
          </div>
                      ...
                      {post.tags && post.tags.length > 0 && (
                        <span className="insights-article-tags">
                          {post.tags.slice(0, 2).map((tag) => (
                            <span key={tag} className="insights-tag-badge">
                              {tag}
                            </span>
                          ))}
                        </span>
                    ...
                    {isLoadingMore ? (
                      ...
                      <>
                        ...
                        <Icon
                          ...
                          className="insights-load-more-arrow"
                          width={18}
                          height={18}
                        />
                      </>
      ...
      {isEditorOpen && (
        <div
          className="insights-editor-overlay"
          role="dialog"
          aria-modal="true"
          aria-label={editorPost ? 'Edit post' : 'Create post'}
        >
          <div className="insights-editor-modal">
            <div className="insights-editor-header">
              <div>
                <span className="insights-editor-eyebrow">
                  {editorPost ? 'BACK TO THE DRAFT' : 'A FRESH PAGE'}
                </span>
              ...
              <button
                className="insights-editor-close"
            ...
            <div className="insights-editor-fields">
              <div className="insights-editor-section-label insights-editor-full">
                ...
                <div>
                  ...
                  <small>Give the piece a name and a little context.</small>
                </div>
              </div>
              <label>
                <span>Title</span>
              ...
              <label>
                ...
                <input
                  ...
                  onChange={(e) => setEditorForm((prev) => ({ ...prev, slug: e.target.value }))}
              ...
              <div className="insights-editor-section-label insights-editor-full insights-editor-section-spaced">
                ...
                <div>
                  ...
                  <small>Organize it now; you can always change it later.</small>
                </div>
              </div>
              <label>
                <span>Category</span>
              ...
              <label>
                ...
                <input
                  ...
                  onChange={(e) => setEditorForm((prev) => ({ ...prev, tags: e.target.value }))}
                  placeholder="React, Supabase, Notes"
                />
              </label>
              <label>
                <span>Thumbnail URL</span>
                <input
                  value={editorForm.thumbnail}
                  onChange={(e) =>
                    setEditorForm((prev) => ({ ...prev, thumbnail: e.target.value }))
                  }
                />
              </label>
              <label>
                ...
                <input
                  value={editorForm.cover_image}
                  onChange={(e) =>
                    setEditorForm((prev) => ({ ...prev, cover_image: e.target.value }))
                  }
                />
              </label>
              <label>
                ...
                <input
                  value={editorForm.reading_time}
                  onChange={(e) =>
                    setEditorForm((prev) => ({ ...prev, reading_time: e.target.value }))
                  }
              ...
              <label>
                ...
                <input
                  type="number"
                  value={editorForm.display_order}
                  onChange={(e) =>
                    setEditorForm((prev) => ({ ...prev, display_order: Number(e.target.value) }))
                  }
                />
              </label>
            </div>
            ...
            <div className="insights-editor-content">
              <div className="insights-editor-writing-label">
                <span>03</span>
                <div>
                  <strong>The actual writing</strong>
                  <small>This is the good part.</small>
                </div>
              </div>
              <div className="insights-editor-content-header">
                ...
                {editorMode === 'rich' && (
                  <div className="insights-editor-toolbar">
                    <button onClick={() => formatRichText('formatBlock', 'H2')} title="Heading">
                      H
                    </button>
                    <button onClick={() => formatRichText('bold')} title="Bold">
                      <strong>B</strong>
                    </button>
                    <button onClick={() => formatRichText('italic')} title="Italic">
                      <em>I</em>
                    </button>
                    <button
                      onClick={() => formatRichText('formatBlock', 'BLOCKQUOTE')}
                      title="Quote"
                    >
                      ❝
                    </button>
                    ...
                    <button
                      onClick={() => {
                        ...
                        const image = selection?.anchorNode?.parentElement?.querySelector(
                          'img:last-of-type',
                    ...
                    <button onClick={() => formatRichText('removeFormat')} title="Clear formatting">
                      Tx
                    </button>
                  </div>
                )}
              ...
              {editorMode === 'rich' ? (
                <div
                  ...
                  onInput={(e) =>
                    setEditorContent(htmlToMarkdown((e.currentTarget as HTMLDivElement).innerHTML))
                  }
            ...
            <div className="insights-editor-options">
              <label>
                <input
                  ...
                  onChange={(e) =>
                    setEditorForm((prev) => ({ ...prev, published: e.target.checked }))
                  }
                />{' '}
                Published
              </label>
              <label>
                <input
                  ...
                  onChange={(e) =>
                    setEditorForm((prev) => ({ ...prev, featured: e.target.checked }))
                  }
                />{' '}
                Featured
              </label>
            ...
            <div className="insights-editor-footer">
              <button className="insights-editor-cancel" onClick={() => setIsEditorOpen(false)}>
                Cancel
              </button>
              <button
                className="insights-editor-save"
